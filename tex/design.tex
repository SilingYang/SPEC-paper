...
Next, \pname{} employs the I/O-efficient ITF method to identify the important tokens within $R$, assuming a subset $R_{\text{important}}$ is identified (\cref{sec:techa}). 
If KVs in $R_{\text{important}}$ are not in GPU memory, they are retrieved from lower memory tiers. 
During this process, \pname{} employs layer-aware prefetching to speculatively load the next layer's KVs effectively overlapping I/Os with computation (\cref{sec:technew}).
...
Essentially, $R_{\text{important}}$ plus $NR$ becomes the de facto prefix used in the LLM inference. 
