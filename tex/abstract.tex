...
%model inference. 
\zrdnew{In this paper, we present \pname{}, an importance-informed multi-tier prefix KV caching and prefetching system designed to reduce TTFT in LLM inference. \pname{} first employs an I/O-efficient algorithm to identify and load only the most important KVs, thereby minimizing I/O overhead. By exploiting the similarity of important tokens across adjacent layers, it speculatively prefetches the next layer's KVs during the current layer's computation, effectively overlapping slow disk I/O with GPU execution. Finally, \pname{} optimizes prefix KV storage and cache utilization through importance-informed KV management, achieving additional reductions in TTFT for end-to-end inference.}
Our experimental results show that \pname{} can reduce TTFT by
up to \zrdnew{1.75}$\times$ compared to state-of-the-art systems, while maintaining
