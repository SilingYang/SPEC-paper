...
\zrdnew{For layer-aware prefetching, the runtime verification incurs negligible latency. This operation involves simple index comparisons executed in microseconds, which is orders of magnitude faster than the millisecond-level disk I/O latency it helps to avoid.}
...
\zrdnew{The prefetch buffer is small because it only stores one layer of KVs at a time. It does not take up space from the main KV cache.}
