...
%}
\zrdnew{\textbf{Design.} Based on the above observations, we propose the layer-aware KV prefetching mechanism. The core idea is that owing to the high similarity of important token index sets across adjacent layers, we can utilize the importance distribution of the current layer to approximate and speculatively prefetch the critical KVs for the subsequent layer. This enables the system to proactively load the next layer's KVs using idle PCIe bandwidth while the current layer is still being computed. By overlapping these I/O operations with GPU execution, HyperInfer effectively hides the data loading latency and eliminates the GPU stalls typically caused by the dependency between token identification and KV loading.}
...
\zrdnew{While layer-aware KV prefetching effectively hides I/O latency, designing an efficient mechanism requires addressing two key challenges. First, using the current layer's important tokens to approximate those of the next layer is inherently speculative. Although similarity is high, inaccuracies are inevitable, necessitating a robust verification and recovery mechanism to ensure correctness without incurring high penalties. Second, prefetching I/O must be carefully scheduled. Since this is an opportunistic optimization utilizing idle PCIe bandwidth parallel to computation, the system must ensure that speculative loads do not interfere with the critical execution path or delay the subsequent demand loading of missing KVs.}

\zrdnew{
To address the first issue, we implement a speculate-then-verify workflow. During the computation of the current layer (Layer $i$), HyperInfer speculatively loads the KVs for Layer $i+1$ based on the importance ranking. As the execution transitions to Layer $i+1$, the system performs a lightweight verification step: it executes the similarity-guided identification (see \S IV-B) using the probe heads to determine the ground-truth important tokens. HyperInfer then calculates the set difference between the ground-truth set and the already prefetched KVs. Only the missing KVs (the delta) are synchronously loaded on the critical path. Since the volume of these missing KVs is typically much smaller than the full set required for demand loading, this recovery step incurs significantly lower latency. Even in the worst-case scenario where prefetching fails completely, the system simply falls back to standard demand loading, ensuring that the performance is bounded.}
...
Second, to prevent resource contention, we execute prefetching commands via asynchronous CUDA streams. This design isolates the I/O operations on the PCIe bus, which is orthogonal to the on-chip HBM bandwidth heavily utilized by the computation kernels. Consequently, speculative prefetching can saturate idle I/O bandwidth without interfering with the critical path execution of the current layer.}
...
\zrdnew{\textbf{Handling Mispredictions.} 
	...
	First, it ensures \textit{performance determinism}. 
	Including extra KVs would dynamically increase the sequence length, introducing variable computational overhead and memory consumption that could negate the latency benefits of prefetching. 
	Second, it maintains \textit{resource efficiency}. 
	Retaining mispredicted KVs would unnecessarily increase memory consumption and complicate runtime data management.
	Although this approach discards data transferred via PCIe, this bandwidth was otherwise idle; thus, the speculation incurs \textit{zero opportunity cost} to the critical inference path while maintaining a rigorous and fair evaluation baseline.}
