...
First, to determine the safe prefetching window, we leverage the structural homogeneity of Transformer layers. We employ a runtime heuristic that estimates the current layer's computation time using an exponential moving average  of the execution times from previous layers. This approach filters out transient system jitter and provides a robust time budget. To ensure safety, we set the prefetching deadline conservatively (e.g., 95\% of the estimated window) to prevent I/O from extending onto the critical path.
%Second, to prevent resource contention, we execute prefetching commands via asynchronous CUDA streams. 
Second, to mitigate resource contention, we issue prefetching commands through asynchronous CUDA streams. This isolates PCIe-based I/O from the HBM bandwidth heavily utilized by compute kernels, as the two resources are largely orthogonal.  
%This design isolates the I/O operations on the PCIe bus, which is orthogonal to the HBM bandwidth heavily utilized by the computation kernels. 
As a result, speculative prefetching can saturate idle I/O bandwidth without interfering with the critical path execution of the current layer.}
...
(1)It ensures performance determinism. Including mispredicted KVs causes sequence lengths to fluctuate based on prediction accuracy, introducing latency jitter that jeopardizes Service-Level Agreements (SLAs). By discarding them, we maintain a stable computational workload.
	%This leads to unpredictable latency jitter. Discarding them ensures the workload remains stable, which is crucial for meeting Service-Level Agreements (SLAs).
(2) It prevents overhead from negating prefetching benefits.Incorporating extra tokens incurs non-negligible overhead: longer sequences increase attention matrix computation time, which can offset or even erase the latency savings from I/O hiding. Discarding them ensures we hide I/O latency without slowing down the GPU computation.
