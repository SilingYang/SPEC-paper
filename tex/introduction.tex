...
\zrdnew{Typically, the LLM inference process comprises two distinct phases: the prefill phase which processes the input prompt to generate the first token, and the decoding phase which generates subsequent tokens autoregressively~\cite{distserve-osdi24, loongserve-arxiv24, splitwise-isca24}.}
While longer requests enhance the quality of the model's output, they also
significantly increase prefill latency, particularly the time to
...
reduction. \zrdnew{To address capacity limitations,  AttentionStore~\cite{attentionstore-atc24} expand KV storage to disks. Yet, limited disk bandwidth  becomes a bottleneck, making retrieval latency from SSD to GPU a dominant factor in TTFT as shown in~\ref{sec:limit}. 
IMPRESS~\cite{impress-fast25} attempts to mitigate this by leveraging token importance insights~\cite{h2o-nips23, scissorhands-nips23} to selectively load only critical KVs. However, since the system cannot identify the next layer's important KVs until the current layer's computation finishes, I/O operations are forced to be serial. This prevents effective overlapping of I/O with computation, leaving latency exposed on the critical path and limiting further TTFT reduction.}

\zrdnew{To break this serial dependency, we propose \pname{}, an importance-informed multi-tier prefix KV caching and prefetching system. 
The core idea is to load only the KVs of important prefix tokens, minimizing I/Os from slower disks, and crucially, to speculatively prefetch future critical KVs to overlap I/Os with computation, thereby maximizing latency reduction. However, building such an efficient system involves three significant challenges.}
...
\zrdnew{Second, the identification of important tokens creates a strict read-after-compute dependency that the system cannot identify critical KVs for the next layer until the current layer's computation finishes.
Existing strategies face a dilemma: they either waste I/O bandwidth by aggressively loading all KVs~\cite{attentionstore-atc24}, or suffer from I/O stalls by strictly waiting for identification~\cite{impress-fast25}.
While speculative methods like InfiniGen~\cite{infinigen-osdi24} exist, they target the decoding phase and incur prohibitive overheads for prefix processing.
Hence, a lightweight mechanism is needed to speculatively prefetch prefix KVs, breaking this dependency to hide I/O latency.}
...
\zrdnew{To address the second challenge, we exploit the observation that the sets of important token indices are highly similar between adjacent layers. Based on this property, we introduce the \technew{} mechanism to predict the next layer's important tokens using the current layer's results. This enables speculative prefetching of the next layer's crucial KVs during the current layer's execution to mask I/O latency and significantly reduce TTFT.
}
...
\item \zrdnew{We propose the \technew{} mechanism, which exploits the similarity of important tokens across adjacent layers. This allows the system to speculatively prefetch the next layer's KVs, breaking the serial dependency between token identification and loading to fully overlap I/Os with computation.}
