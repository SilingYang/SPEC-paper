...
significantly increase prefilling latency, particularly the time to
...
\zrdnew{The core idea is to load only the KVs of important prefix tokens, minimizing I/Os from slower disks, and crucially, to speculatively prefetch future critical KVs to overlap I/Os with computation, thereby maximizing latency reduction. However, building such an efficient system involves three significant challenges.}
...
\item \cp{We propose the \technew{} mechanism, which exploits the similarity of important tokens across adjacent layers. This allows the system to speculatively prefetch the next layer's KVs, breaking the serial dependency between token identification and loading to fully overlap I/Os with computation.}
